/* Amicro — AnimatedButton. MIT © 2026 Syed Subhan Uddin — github.com/Subhan-code/Amicro--Micro-transitions-
   Copied from src/components/AnimatedButton.tsx; j.ai edits are marked. */
import React, { useState } from 'react';
import { AnimatePresence, motion } from 'motion/react';
import { ButtonConfig } from './buttons-data';
...
export const AnimatedButton = React.memo(function AnimatedButton({ config, layoutMode, theme = 'dark', onClick, className = '' }: AnimatedButtonProps) {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.96 }}
      className={`relative flex items-center justify-center rounded-[40px] border-0 cursor-pointer shadow-none transition-colors duration-150 ${isMatrix ? 'w-[36px] h-[36px] sm:w-[42px] sm:h-[42px] px-0' : 'h-[36px] min-w-[75px]'} ${config.interactionType === 'glare' ? 'overflow-hidden' : ''} ${isLightTheme ? 'text-black' : 'text-[#e3e3e3]'} ${className}`}
    >
